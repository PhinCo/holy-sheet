...
class XlsxReader extends EventEmitter{
	...
	async readEntireFile(){
		return new Promise( ( resolve, reject ) => {
			const workSheet = xlsx.readFile( this.xslxFilePath, this.options );
			...
			for( let n = 1; n < sheet.rows.length; n++ ){
				...
				_.each( columnNames, ( columnName, index ) => {
					newRow[columnName] = rawRow[index];
				});
	...
	chooseSheetToWorkWith( sheets ){
		...
		if( !sheet ) throw new Error("Counldn't find your sheet -> " + Object.keys( workSheet.Sheets ).join(', ') );
