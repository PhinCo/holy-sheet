...
class EnumTransformer extends AbstractTransformer{
	...
	constructor( config ){
		 ...
		 */
		this.outputDelimeter = _.has( config, 'outputDelimeter' ) ? config.outputDelimeter : null;
	}

	transform( input ){
		if( input === null || input === void 0 ) return null;
		
		const value = super.transform( input );
		...
		return this.outputDelimeter ? values.join( this.outputDelimeter ) : values;
	}
