	...
	transform(){
		...
		const transformedRows = _.map( self.fileReadResult.dataRows, row => {
			...
			if( _.isFunction( self.transformer.visitRow ) ){
				...
				return self.transformer.visitRow( output, self.fileReadResult.extractions );
			}

			output.__errors = rowErrors;

			if( rowErrors.length > 0 ) allErrors = allErrors.concat( rowErrors );
