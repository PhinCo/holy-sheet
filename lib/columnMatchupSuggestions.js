...
export function _bestMatchesByName( columnTransformer, inputColumnNames, rows ){
	...
	const results = _.map( inputColumnNames, inputColumnName => {
		...
		if( columnTransformer.columnNameAliases ) names = [columnTransformerName, ...columnTransformer.columnNameAliases];

		let maxSimilarity = 0;
		for( let name of names ){
			const similarity = stringSimilarity.compareTwoStrings( inputColumnName, name );
